...
void create_recommend(string id) {
	...
	if (truncate_id != ""){
		map<string, int>::iterator it = Accounts.find(truncate_id);
		if (it == Accounts.end() || it->second == -1)
			result.push_back(truncate_id);
	}
	//the same length as id && length +1
/*	map<string, int>::iterator it2 = Accounts.find(id);
...
*/
	for (int i = 0; i < 62; i++) {
		map<string, int>::iterator it1 = Accounts.find(truncate_id + PostFix[i]);
		if (it1 == Accounts.end() || it1->second == -1) 
		{
			...
			for (int j = 0; j < 62; j++) {
				map<string, int>::iterator it2 = Accounts.find(id + PostFix[j]);
